...
const KIND_LABEL={'fixed-strict':'Fixed stress · strict set','fixed-dominant':'Fixed stress · dominant set only','fixed-unreviewed':'Fixed stress (not yet reviewed)','phrase-lumped':'Fixed stress, word vs phrase not separated','weight':'Weight-sensitive','weight-hedged':'Weight-sensitive (stated with hedges)','weight-unreviewed':'Weight-sensitive (editor\'s review)','lexical':'Lexical (unpredictable)','morphological':'Set by morphology','variable':'Variable','tonal':'Tone or pitch accent','no-word-stress':'No word stress','phrasal':'Phrase-level only','segmental':'Depends on segments','conflict':'Descriptions disagree','undetermined':'Not determinable from sources','excluded':'Other (see record)','other':'Other','—':'Outside the catalog'};
const KIND_ORDER=Object.keys(KIND_LABEL);
const CAT_LABEL={'in':'In the catalog','out':'Outside the catalog'};
const AUDIT_LABEL={'well-sourced':'Well sourced','adequate':'Adequate','thin':'Thin','at-risk':'At risk','no-stress-source-exists':'No stress source exists','—':'Not audited (outside the catalog)'};
const AUDIT_ORDER=['well-sourced','adequate','thin','at-risk','no-stress-source-exists','—'];
const ACCESS={digital:{label:'Digital',cls:'a-digital',long:'an open copy, a copy on file, or a copy behind a Harvard login'},scan_deliver:{label:'Scan & Deliver',cls:'a-scan',long:'Harvard holds a print copy: articles and chapters can be scanned on request, whole books are a loan'},print_only:{label:'Not at Harvard',cls:'a-print',long:'located (a print copy, or a publisher record behind a paywall), but no open copy and none at Harvard'},unknown:{label:'Unknown',cls:'a-unknown',long:'not located, or not yet checked'}};
const ACCESS_KEYS=['digital','scan_deliver','print_only','unknown'];
const BEST_LABEL={digital:'At least one digital copy',scan_deliver:'Harvard print copy, nothing digital',print_only:'Located, but not at Harvard',unknown:'Nothing located or checked',none:'No works listed'};
const BEST_ORDER=['digital','scan_deliver','print_only','unknown','none'];
const NEW_LABEL={verified:'Found and verified by a second reader',found:'Found, second reading pending or failed',none:'None'};
const NEW_ORDER=['verified','found','none'];
const WORLD_ORDER=['statement found in an open source','statement found in a copy already on file','statement reported second-hand only','a description exists but is not accessible','accessible description is silent on stress','no descriptive source known','not yet read','—'];
const WORLD_LABEL={'—':'In the catalog (not part of the reading pass)'};
const SWEEP_ORDER=['new statement read','swept, no new statement','not swept','—'];
const SWEEP_LABEL={'—':'No stress-titled works listed'};
const FOUND_BY={'world pass':'found in the reading pass','stress-title sweep':'found in the sweep of stress-titled works'};
const COLL=new Intl.Collator('en',{sensitivity:'base',numeric:true});
const ORIGIN_LABEL={glottolog:'listed by Glottolog',reader:'found by a reader',scan:'named by the earlier scan',database:'from the catalog record'};
const GROUPS=[
 ...
 ['bears','Read: bears on the catalog coding',['read: confirms the catalog coding','read: refines the catalog coding','read: contradicts the catalog coding'],true],
 ...
 ['wanted','Wanted: likely states stress, not held',['wanted: likely states stress, not held'],true],
 ...
 ['scan','Named by the earlier scan, not opened',['named by the earlier scan, not opened'],false],
 ['other','Other',[],false]];
...
const COLS=[['name','Language'],['family','Family'],['catlabel','Catalog'],['status','Status'],['n_sources','Works'],['n_states_stress','What we know'],['digital','Access']];
...
function renderChips(){ const c=$('chips'); c.innerHTML=''; const add=(label,fn)=>{ const s=document.createElement('span'); s.className='chip'; s.innerHTML=esc(label)+' <button aria-label="Remove">×</button>'; s.querySelector('button').onclick=()=>{ fn(); state.page=0; render(); }; c.appendChild(s); };
  ...
  state.cat.forEach(k=>add(CAT_LABEL[k],()=>state.cat.delete(k))); state.best.forEach(k=>add('access: '+BEST_LABEL[k],()=>state.best.delete(k))); state.newst.forEach(k=>add('new statement: '+NEW_LABEL[k],()=>state.newst.delete(k))); state.audit.forEach(k=>add('audit: '+(AUDIT_LABEL[k]||k),()=>state.audit.delete(k))); state.world.forEach(k=>add('reading pass: '+(WORLD_LABEL[k]||k),()=>state.world.delete(k))); state.sweep.forEach(k=>add('title sweep: '+(SWEEP_LABEL[k]||k),()=>state.sweep.delete(k))); state.kind.forEach(k=>add(KIND_LABEL[k]||k,()=>state.kind.delete(k))); state.area.forEach(k=>add(k,()=>state.area.delete(k))); if(state.fam) add(state.fam,()=>state.fam='');
...
function renderRail(){ const rail=$('rail'); const keep=rail.scrollTop; rail.innerHTML='';
  ...
  mk('Catalog','cat','cat',['in','out'],CAT_LABEL);
  mk('Access to the works listed','best','best',BEST_ORDER,BEST_LABEL);
  mk('New stress statement from the reading passes','newst','newst',NEW_ORDER,NEW_LABEL);
  mk('Audit of the catalog record','auditk','audit',AUDIT_ORDER,AUDIT_LABEL);
  mk('Reading pass, languages outside the catalog','worldk','world',WORLD_ORDER,WORLD_LABEL);
  mk('Sweep of stress-titled works','sweepk','sweep',SWEEP_ORDER,SWEEP_LABEL);
...
/* ---- drawer ---- */
const cache=new Map();
function metaHTML(e){ return `<h2>${esc(e.name)}</h2><div class="meta"><a class="ext" href="${GL(e.glottocode)}" target="_blank" rel="noopener" title="Glottolog: classification, location, full bibliography">Glottolog ${esc(e.glottocode)}</a><span>${esc(e.family||'—')}</span><span>${esc(e.macroarea||'—')}</span><span>${e.in_catalog?'in the catalog':'outside the catalog'}</span></div>`; }
...
function stmtHTML(s){ const ok=!!s.verified; const head=[ok?'<span class="tag ok">Verified by a second reader</span>':`<span class="tag ${/pending/.test(s.check||'')?'muted':'warn'}">${esc(s.check||'second reading pending')}</span>`, s.found_by?esc(FOUND_BY[s.found_by]||('found in the '+s.found_by)):'' , s.source_kind?esc(s.source_kind):'', s.year?esc(s.year):''].filter(Boolean).join(' · ');
...
function fullHTML(e,L){ const kv=[]; const push=(k,v)=>{ if(v!==undefined&&v!==null&&String(v).trim()!=='') kv.push(`<dt>${k}</dt><dd>${v}</dd>`); };
  if(e.in_catalog){ push('Classification',`<span class="tag k-${esc(e.catalog_kind)}">${esc(e.catlabel)}</span>`);
    const a=L.audit||{}; if(a.status||e.audit_status){ let h=`<b>${esc(AUDIT_LABEL[a.status||e.audit_status]||a.status||e.audit_status)}</b>`; const act=a.action||e.audit_action; if(act) h+=' — '+esc(act); if(a.second_reader&&a.second_reader!=='not run') h+=`<div class="cite">second reader: ${esc(a.second_reader)}${a.quote_check?' · quotation check: '+esc(a.quote_check):''}</div>`; else if(a.quote_check) h+=`<div class="cite">quotation check: ${esc(a.quote_check)}</div>`; if(a.deciding_source_kind||a.deciding_source_year) h+=`<div class="cite">deciding source: ${esc([a.deciding_source_kind,a.deciding_source_year].filter(Boolean).join(', '))}</div>`; if(a.recode_to) h+=`<div class="cite"><b>Recode to:</b> ${esc(a.recode_to)}</div>`; if(a.reasoning) h+=`<details class="more"><summary>Reader's reasoning</summary>${esc(a.reasoning)}</details>`; if(a.second_reader_evidence) h+=`<details class="more"><summary>Second reader's evidence</summary>${esc(a.second_reader_evidence)}</details>`; push('Audit',h); } }
  else { if(e.world_status) push('Reading pass',esc(e.world_status)); const sc=L.earlier_scan||{}; if(sc.verdict||e.scan_verdict){ let h=esc(sc.verdict||e.scan_verdict); if(sc.source_named) h+=`<div class="cite">source named: ${linkify(sc.source_named)}</div>`; if(sc.note) h+=`<div class="cite">${linkify(sc.note)}</div>`; push('Earlier scan',h); } }
  if(e.sweep_status){ let h=esc(e.sweep_status); if(L.sweep_summary) h+=`<details class="more"><summary>Reader's summary</summary>${esc(L.sweep_summary)}</details>`; push('Stress-titled works',h); }
  const c=L.counts||e; push('Works listed',`<div>${plural(c.n_sources,'work')}${c.n_glottolog_entries?` · Glottolog lists ${plural(c.n_glottolog_entries,'entry').replace('entrys','entries')} of any kind`:''}</div>${c.n_sources?accessLine(c):''}${c.n_sources?`<div class="cite">${fmt(c.n_states_stress)} state the stress pattern · ${fmt((c.n_read_silent||0)+(c.n_read_other||0))} read without a deciding statement · ${fmt(c.n_wanted)} wanted, not held · ${fmt(c.n_unread)} not yet read</div>`:''}`);
  ...
  const ns=(L.new_statements||[]).slice().sort((a,b)=>(b.verified?1:0)-(a.verified?1:0)); ns.forEach(s=>st+=stmtHTML(s));
...
function toCSV(rows){ const base=location.href.split('#')[0]; const cols=[['language',e=>e.name],['glottocode',e=>e.glottocode],['glottolog_url',e=>GL(e.glottocode)],['family',e=>e.family],['macroarea',e=>e.macroarea],['in_catalog',e=>e.in_catalog?'yes':'no'],['catalog_classification',e=>e.catalog_kind],['audit_status',e=>e.audit_status],['audit_action',e=>e.audit_action],['reading_pass_outcome',e=>e.world_status],['title_sweep',e=>e.sweep_status],['earlier_scan',e=>e.scan_verdict],['works_listed',e=>e.n_sources],['states_stress',e=>e.n_states_stress],['read_silent',e=>e.n_read_silent],['read_other',e=>e.n_read_other],['wanted',e=>e.n_wanted],['not_yet_read',e=>e.n_unread],['digital',e=>e.digital],['scan_deliver',e=>e.scan_deliver],['print_only',e=>e.print_only],['unknown',e=>e.unknown],['best_access',e=>BEST_LABEL[e.best]],['glottolog_entries',e=>e.n_glottolog_entries],['new_statement',e=>e.has_new_statement?'yes':'no'],['new_statement_verified',e=>e.new_statement_verified?'yes':'no'],['deciding_quote',e=>e.best_quote],['bibliography_url',e=>base+'#'+e.glottocode]]; const q=v=>{ const s=String(v??''); return /[",\n]/.test(s)?'"'+s.replace(/"/g,'""')+'"':s; }; return cols.map(c=>c[0]).join(',')+'\n'+rows.map(e=>cols.map(c=>q(c[1](e))).join(',')).join('\n'); }
...
function legend(){ const inC=D.filter(e=>e.in_catalog).length, outC=D.length-inC, works=D.reduce((s,e)=>s+(e.n_sources||0),0), verified=D.filter(e=>e.new_statement_verified).length;
  $('legend').innerHTML=`<b>How to read this.</b> One row per language: the ${fmt(inC)} languages in the catalog and ${fmt(outC)} other languages surveyed alongside it, with every work that describes, or could describe, the language's word stress — ${plural(works,'work')} in all. Click a language for the stress statements found in its sources and the full list of works, grouped by what we know about each. <b>Read</b> means a reader opened the text and reported what it says about stress; <b>verified</b> means a second reader found the quoted passage at the stated page (${fmt(verified)} languages have a verified new statement so far). <b>Wanted</b> works are ones a reader judged likely to state the stress pattern but which we do not hold. How a copy can be had, per work: <b>Digital</b> — an open copy, a copy on file, or a copy behind a Harvard login; <b>Scan &amp; Deliver</b> — Harvard holds a print copy (articles and chapters can be scanned on request; whole books are a loan); <b>Not at Harvard</b> — located (a print copy, or a publisher record behind a paywall), but no open copy and none at Harvard; <b>Unknown</b> — not located, or not yet checked. Search matches language names, glottocodes, families and the authors of the works listed.`;
  $('edition').textContent=`Bibliography · ${fmt(D.length)} languages · ${plural(works,'work')}${BUILT?' · built '+BUILT.slice(0,10):''}`;
  $('provnote').textContent=`Works come from Glottolog 5.1's bibliography for each language, from the catalog's own records and from the readers' reports; access was checked against Harvard's library catalogue (HOLLIS), HathiTrust, the Internet Archive and open-access indexes. Works are matched across those lists by first author, year and title, so a citation copied differently may appear twice. Access classes for works not yet run through the library check are 'unknown' unless Glottolog records an open link.${BUILT?' Data built '+BUILT.replace('T',' ').replace('Z',' UTC')+'.':''} Corrections: open an issue at github.com/mattschwartzscience/word-stress-catalog.`; }
