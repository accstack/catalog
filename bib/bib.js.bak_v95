...
function fullHTML(e,L){ const kv=[]; const push=(k,v)=>{ if(v!==undefined&&v!==null&&String(v).trim()!=='') kv.push(`<dt>${k}</dt><dd>${v}</dd>`); };
  ...
  return metaHTML(e)+`<dl class="kv">${kv.join('')}</dl>`+stHead+(st||'<p class="muted">No first-hand statement about stress has been found for this language.</p>')+bibHTML(L); }
