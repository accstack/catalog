...
/* the source audit rated the first catalog records only; a language recorded since has no audit status ('later') and is rated by its confidence level instead; '—' = no stress record yet */
const AUDIT_LABEL={'well-sourced':'Well sourced','adequate':'Adequate','thin':'Thin','at-risk':'At risk','no-stress-source-exists':'No stress source exists','later':'Recorded after the source audit; see its confidence level','—':'No stress record yet'};
const AUDIT_ORDER=['well-sourced','adequate','thin','at-risk','no-stress-source-exists','later','—'];
let N_AUDITED=0;   // recorded languages with an audit status, counted when the index loads (the facet title)
/* confidence (Matt's five levels, 2026-09-18): e.confidence = 4..0 or null (not yet rated), e.confidence_why = the one-sentence reason; recorded languages only */
...
const CF_MEANING={'4':'clear consensus','3':'few sources or mild disagreement but a clear winner','2':'real disagreement but we think we know the answer','1':'authoritative sources disagree','0':'no confirmed first-hand statement, essentially a guess','x':'the judged reading that sets the middle levels has not reached this record yet'};
const cfCls=k=>'cf-'+k;
function cfBadge(e){ const k=e.confk; if(k==='—') return ''; return `<span class="tag ${cfCls(k)}" title="${esc(CF_MEANING[k]||'')}">${esc(CF_LABEL[k])}</span>`; }
/* access classes (Matt's definitions, 2026-09-18): a work is 'digital' when a digital copy is known to exist anywhere; digital_gated=true marks a copy behind a login, a paywall, HathiTrust search-only viewing or a restricted repository. Counts: digital = every digital copy, digital_gated = the gated ones among them. */
const ACCESS={digital:{label:'Digital',cls:'a-digital',long:'a digital copy is known to exist anywhere: open (read, or on file with the project)'},digital_gated:{label:'Digital (gated)',cls:'a-digital a-gated',long:'a digital copy is known to exist, but behind a login, a paywall, HathiTrust search-only viewing or a restricted repository'},scan_deliver:{label:'Scan & Deliver',cls:'a-scan',long:'to our knowledge the work has never been scanned into a PDF: no digital copy exists anywhere, and a library holds a print copy that can be scanned on request'},print_only:{label:'Print only',cls:'a-print',long:'no digital copy known and no library print holding we can request from (located in print only, e.g. a catalogue record elsewhere)'},unknown:{label:'Unknown',cls:'a-unknown',long:'nothing located or not checked'}};
const ACCESS_KEYS=['digital','digital_gated','scan_deliver','print_only','unknown'];
const accCounts=o=>({digital:(o.digital||0)-(o.digital_gated||0),digital_gated:o.digital_gated||0,scan_deliver:o.scan_deliver||0,print_only:o.print_only||0,unknown:o.unknown||0});   // open digital copies, then the gated ones
const BEST_LABEL={digital:'At least one digital copy',digital_gated:'Digital, gated only',scan_deliver:'Library print copy, nothing digital',print_only:'Located, print only',unknown:'Nothing located or checked',none:'No works listed'};
...
const state={q:'',terms:[],cat:new Set(),conf:new Set(),best:new Set(),newst:new Set(),audit:new Set(),world:new Set(),sweep:new Set(),kind:new Set(),area:new Set(),fam:'',sort:'name',dir:1,page:0,sel:''};
const PAGE=100;
let current=[];
...
function prep(e){
  ...
  e.status=e.in_catalog?(AUDIT_LABEL[e.audit_status]||e.audit_status||'recorded after the source audit'):(e.world_status||'');
  e.n_read=(e.n_read_silent||0)+(e.n_read_other||0);
  e.hay=(e.name+' '+e.glottocode+' '+e.family+' '+e.macroarea).toLowerCase();
}
function reset(){ state.q=''; state.terms=[]; ['cat','conf','best','newst','audit','world','sweep','kind','area'].forEach(k=>state[k]=new Set()); state.fam=''; $('q').value=''; }
function match(e){
...
function renderChips(){ const c=$('chips'); c.innerHTML=''; const add=(label,fn)=>{ const s=document.createElement('span'); s.className='chip'; s.innerHTML=esc(label)+' <button aria-label="Remove">×</button>'; s.querySelector('button').onclick=()=>{ fn(); state.page=0; render(); }; c.appendChild(s); };
  ...
  state.cat.forEach(k=>add(CAT_LABEL[k],()=>state.cat.delete(k))); state.conf.forEach(k=>add('confidence: '+(CF_LABEL[k]||k),()=>state.conf.delete(k))); state.best.forEach(k=>add('access: '+BEST_LABEL[k],()=>state.best.delete(k))); state.newst.forEach(k=>add('new statement: '+NEW_LABEL[k],()=>state.newst.delete(k))); state.audit.forEach(k=>add('source audit: '+(AUDIT_LABEL[k]||k),()=>state.audit.delete(k))); state.world.forEach(k=>add('what the readers found: '+(WORLD_LABEL[k]||k),()=>state.world.delete(k))); state.sweep.forEach(k=>add('stress-titled works: '+(SWEEP_LABEL[k]||k),()=>state.sweep.delete(k))); state.kind.forEach(k=>add(KIND_LABEL[k]||k,()=>state.kind.delete(k))); state.area.forEach(k=>add(k,()=>state.area.delete(k))); if(state.fam) add(state.fam,()=>state.fam='');
...
function renderRail(){ const rail=$('rail'); const keep=rail.scrollTop; rail.innerHTML='';
  /* hide: a key left out of the list (its languages are still counted in the other facets); tagcls: k -> a class that renders the label as a coloured tag; always: keys listed even at a count of 0 */
  ...
  mk('Confidence','confk','conf',CF_ORDER,CF_LABEL,'—',cfCls,CF_ORDER);   /* all six levels listed, in Matt's order, even at 0; languages without a record are left out of its counts */
  ...
  mk(`Source audit of the first ${fmt(N_AUDITED)} records`,'auditk','audit',AUDIT_ORDER,AUDIT_LABEL);
...
function legend(){ const inC=D.filter(e=>e.in_catalog).length, outC=D.length-inC, works=D.reduce((s,e)=>s+(e.n_sources||0),0), verified=D.filter(e=>e.new_statement_verified).length;
  $('legend').innerHTML=`<b>How to read this.</b> One row per language: the ${fmt(inC)} languages whose stress pattern is recorded in the catalog and ${fmt(outC)} whose stress is not yet recorded (their bibliography is here; the readers are working through their bibliographies), with every work that describes, or could describe, the language's word stress — ${plural(works,'work')} in all. Click a language for the stress statements found in its sources and the full list of works, grouped by what we know about each. <b>Read</b> means a reader opened the text and reported what it says about stress; <b>verified</b> means a second reader found the quoted passage at the stated page (${fmt(verified)} languages have a verified new statement so far). <b>To obtain</b> works are ones a reader judged likely to state the stress pattern that the project has not yet seen. <b>Confidence</b> is the catalog's rating of each recorded language's classification: very confident (clear consensus), fairly confident (few sources or mild disagreement but a clear winner), moderately confident (real disagreement but we think we know the answer), disputed (authoritative sources disagree), not confident (no confirmed first-hand statement, essentially a guess), or not yet rated; the source audit rated the first ${fmt(N_AUDITED)} records only, so a language recorded since is rated by its confidence level alone. How a copy can be had, per work: <b>Digital</b> — a digital copy is known to exist anywhere, open (read or on file) or gated (login, paywall, HathiTrust search-only, restricted repository). <b>Scan &amp; Deliver</b> — to our knowledge the work has never been scanned into a PDF: no digital copy exists anywhere, and a library holds a print copy that can be scanned on request. <b>Print only</b> — no digital copy known and no library print holding we can request from (located in print only, e.g. a catalogue record elsewhere). <b>Unknown</b> — nothing located or not checked. <b>Digital (gated)</b> marks a copy that exists behind a login, a paywall, HathiTrust search-only viewing or a restricted repository: a HathiTrust search-only volume is Digital (gated), never Scan &amp; Deliver, a dead link with a HathiTrust or publisher record still counts as Digital (gated) when that record shows a digitized copy, and only a dead link with no other evidence is Unknown. Search matches language names, glottocodes, families and the authors of the works listed.`;
...
async function load(){ try{ const r=await fetch(DATA+'index.json'); if(!r.ok) throw new Error(r.status); D=await r.json(); }catch(err){ $('crit').textContent='The bibliography index could not be loaded.'; return; }
  D.forEach(prep); byGC=new Map(D.map(e=>[e.glottocode,e])); N_AUDITED=D.filter(e=>e.in_catalog&&e.audit_status).length;
